...
export function useOAuthFlow({
  ...
  const handleOAuthLogin = async (providerId: string, methodIndex: number) => {
    try {
      setAuthStatus((prev) => ({ ...prev, [providerId]: t("初始化...") }))
      ...
      if (method === "code") {
        setAuthStatus((prev) => ({ ...prev, [providerId]: t("等待授权码...") }))
        setManualCodeState({ providerId, id, instructions })
        setManualCodeInput("")
        return
      }

      setAuthStatus((prev) => ({ ...prev, [providerId]: t("等待浏览器授权...") }))
      ...
      const poll = setInterval(async () => {
        try {
          ...
          if (status.status === "success") {
            ...
            setAuthStatus((prev) => ({
              ...prev,
              [providerId]: "Failed: " + (status.result?.message || t("未知错误")),
            }))
          }
        } catch (e) {
          clearInterval(poll)
          ...
          setAuthStatus((prev) => ({ ...prev, [providerId]: t("轮询授权状态出错") }))
        }
      }, 1000)
      ...
      setTimeout(() => {
        if (pollIntervals.current[providerId] === poll) {
          ...
          delete pollIntervals.current[providerId]
          setAuthStatus((prev) => {
            if (prev[providerId]?.startsWith("Waiting")) return { ...prev, [providerId]: t("已超时") }
            return prev
          })
        }
      }, 120000)
    } catch (e) {
      setAuthStatus((prev) => ({ ...prev, [providerId]: t("启动登录失败") }))
      console.error(e)
    }
  }
  ...
  const handleManualCodeSubmit = async () => {
    ...
    try {
      setAuthStatus((prev) => ({ ...prev, [providerId]: t("验证授权码...") }))
      ...
      if (status.status === "success") {
        ...
        setAuthStatus((prev) => ({
          ...prev,
          [providerId]: "Failed: " + (status.result?.message || t("授权码无效")),
        }))
      }
    } catch (e) {
      console.error("Error submitting code:", e)
      setAuthStatus((prev) => ({ ...prev, [providerId]: t("提交授权码失败") }))
    }
  }
